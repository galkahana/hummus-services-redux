...
router.route('/generation-jobs/delete-jobs')
    .post(authenticate.authenticateOrDie, authorizeOwn(Resources.Job, Actions.Update), asyncHandler(generationJobsController.deleteJobs), root.returnOK)
router.route('/generation-jobs/delete-files')
    .post(authenticate.authenticateOrDie, authorizeOwn(Resources.Job, Actions.Update), asyncHandler(generationJobsController.deleteFiles), root.returnOK)
...
router.route('/users/me/change-username')
    .post(authenticate.authenticateOrDie, authorizeOwn(Resources.User, Actions.Update), asyncHandler(usersController.changeUsername), root.returnOK)
router.route('/users/me/change-password')
    .post(authenticate.authenticateOrDie, authorizeOwn(Resources.User, Actions.Update), asyncHandler(usersController.changePassword), root.returnOK)
...
router.route('/tokens/revoke')
    .post(authenticate.authenticateOrDie, authorizeOwn(Resources.Token, Actions.Update), asyncHandler(tokensController.revoke), root.returnOK)
